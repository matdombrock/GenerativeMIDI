...
module.exports = (track)=>{
  for(let i = 0; i < c.sequence_length; i++){
    ...
    let harmony = random.harmony({note:note,mode:c.mode,key:c.key, weights: c.harmony_weights, limit_to_scale:c.limit_harmony_to_scale});
    ...
    let eventNotes = [];
    if(harmonyProbCheck){
      eventNotes = [note+oct, harmony+oct2];
    }else{
      eventNotes = [note+oct]
    }
